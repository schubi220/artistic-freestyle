...
class artistic:
    def calcT(s: QuerySet, judge: Judge):
        return artistic.__calc(s, judge, "values[start.id].values.get('0',0)+values[start.id].values.get('1',0)+values[start.id].values.get('2',0)")

    def calcP(s: QuerySet, judge: Judge):
        return artistic.__calc(s, judge, "values[start.id].values.get('0',0)+values[start.id].values.get('1',0)+values[start.id].values.get('2',0)")

    def calcD(s: QuerySet, judge: Judge):
        return artistic.__calc(s, judge, "(10-(values[start.id].values.get('0',0)*0.5+values[start.id].values.get('1',0))) if values[start.id].values.get('2',0) < 2 else (10-((values[start.id].values.get('0',0)*0.5+values[start.id].values.get('1',0))/math.sqrt(values[start.id].values.get('2',0))))")
